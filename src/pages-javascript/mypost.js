...
export const showMyPostAfterAuth = () => {
  sectionMyPost.style.display = "flex";
...
export const getCardPost = () => store.collection("userPostsCollection").where("userId","==",userId).get();

const onGetTask = (callback) =>
  store.collection("userPostsCollection").where("userId","==",userId).onSnapshot(callback);
...
window.addEventListener("DOMContentLoaded", async (e) => {
  onGetTask((querySnapshot) => {
    ...
    sectionMyPost.innerHTML += `<div id="headLogoUserContainer" class="head-logo-user-container">
  ...
  <div id="userHead" class="user-head">
    ...
    sectionMyPost.innerHTML += `<input id="testIrPost" type="button" value="Ir a post" />`;
